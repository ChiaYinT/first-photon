...
for i = 1:size(dataset.data,1),
    fprintf('i=%d\n', i);
        ...
        idx = find(dataset.data(i,1,:)>threshold,1);
        if isempty(idx),
            break;
        end
...
figure; imagesc(img_first_photon(:,1:9000,:));
set(gca, 'FontSize', 18);
...
for i = 1:size(dataset.laserPos,1),
    first_photon_dist(1,i) = find(dataset.data(i,1,:) > threshold, 1) * dataset.deltat + dataset.t0;
...
[space_carved_idx] = space_carving_check(X,Y,depth_map, filtered_p, sqrt(8));
